  		...
  		<?php
			...
			if(isset($_FILES['image'])){
	           ...
	           //$uploadfile = $uploaddir . basename($_FILES['image']['name']);
                ...
                $newfilename = $COOKIE_AMEI . '.' . end($temp);
				...
				if(empty($errors)==true){
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $uploaddir . $newfilename)) {
                        $sql = "UPDATE img_ava SET img_name = '$COOKIE_AMEI.$file_ext', img_tmp='$uploaddir' WHERE id_people = (SELECT id FROM peoples WHERE amei = '$COOKIE_AMEI')";
                        $stmt = mysqli_query( $conn, $sql);
                    } else {
                        echo "В данный момент данная функция недоступна";
                    }
				}else{
					print $errors;
				}	
